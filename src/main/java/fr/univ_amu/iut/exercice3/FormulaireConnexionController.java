    ...
    BooleanBinding booleanBinding =
        new BooleanBinding() {
          ...
          protected boolean computeValue() {
            return Bindings.lessThan(champMotDePasse.textProperty().length(), 8).get()
                || champMotDePasse.getText().chars().noneMatch(Character::isUpperCase)
                || champMotDePasse.getText().chars().noneMatch(Character::isDigit);
          }
        };
    boutonOk.disableProperty().bind(booleanBinding);
  }

  /** du mot de passe masqué (autant d'étoiles que de caractères saisis). */
  @FXML
